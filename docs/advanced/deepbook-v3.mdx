...
DeepBook V3 is Sui's native on-chain central limit order book (CLOB). Miru executes all copy trades through this protocol.
...
### Market Orders (Not Used by Miru)
...
<Note>Miru **only places limit orders** to copy maker's exact strategy.</Note>
...
<Tip>
  **Miru always uses DEEP tokens** (`payWithDeep: true`) for lower fees. Ensure
  you have DEEP balance!
</Tip>
...
## Miru's DeepBook Integration

### Position Creation

When maker trades, Miru:
...
Miru builds Sui transactions like:
...
Miru polls DeepBook to sync order counts:
...
<Warning>
  **Never use mainnet until officially announced.** Miru is currently
  testnet-only.
</Warning>
