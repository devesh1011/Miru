...
zkLogin is Sui's innovative authentication system that lets you create and use blockchain wallets with familiar OAuth login—no seed phrases, no private key management.
...
<Info>
  zkLogin is **native to Sui blockchain**. Miru leverages this protocol for
...
```
User → Generate Random Private Key → Derive Address
      → Backup 12/24 Word Seed Phrase
      → Never Lose It Or Funds Are Gone Forever
```
...
- ✅ Familiar OAuth flow
- ✅ Nothing to backup or remember
- ✅ Same wallet every time you sign in
- ✅ Cryptographically secure
- ✅ Fully self-custodial

## Miru's zkLogin Implementation
...
<Warning>
  This data is **encrypted at rest** in Supabase. Still, never share your
  zkLogin session with anyone!
</Warning>
...
4. **Approve OAuth**
   ...
   - Shows permissions request:
     - "Miru wants to access your email"
...
<Tip>
  The URL contains sensitive data. Only paste it in the Miru bot, never share
  publicly!
</Tip>
...
<Note>
  Renewing your session **does NOT change your wallet address**. Same Google
  account = same address forever.
</Note>
...
<AccordionGroup>
  <Accordion title="OAuth redirect not working">
    **Common causes:**
    1. Using in-app Telegram browser → Open in external browser
    2. Didn't copy entire URL → Must include `#` and everything after
    3. Waited too long → Redirect expires in 5 minutes
    
    **Solution:** Start fresh, use external browser, copy full URL quickly.
  </Accordion>

<Accordion title="Session expired">
  **Normal behavior** after 7 days. **Solution:** 1. Go to `/wallet` 2. Tap
  **Sign in with Google** 3. Complete OAuth (same account) 4. Wallet address
  unchanged, session renewed
</Accordion>

<Accordion title="Different wallet address appeared">
  **Likely causes:** 1. Different Google account used 2. Different address_seed
  (shouldn't happen in Miru) **Solution:** - Use the **exact same Google
  account** every time - Contact support if persists
</Accordion>

<Accordion title="Can't sign transactions">
  **Check:** 1. zkLogin session active? (see `/wallet`) 2. Session expired?
  Re-authenticate 3. Capability granted? Check wallet settings **Solution:**
  Verify session, re-auth if needed, ensure capability exists.
</Accordion>
...
✅ You trust OAuth provider (Google)
✅ You don't want to manage seed phrases
✅ Quick testing and prototyping
✅ Mainstream user experience

### When to Use Traditional Wallet

✅ Maximum privacy (no OAuth provider)
✅ Long-term cold storage
✅ You're comfortable with seed phrases
✅ Don't want session expiration
✅ Critical production funds

<Tip>
  For Miru, zkLogin is ideal: easy access, frequent re-authentication is fine,
  and you can always export keys later!
</Tip>
...
- Google sees your email accessed "Miru"
- Google **doesn't** know:
